...
class TestExdApiEtc(unittest.TestCase):
    ...
    def setUp(self):
        ...
        FileHandlerRegistry.register(file_type_name="test", factory=ExdApiSimpleImpl)
    ...
    def test_semicolon_parameters(self):
        ...
        handle = service.Open(
            exd_api.Identifier(url=self._get_example_file_path("example_semicolon.csv"), parameters='{"sep":";"}'),
            self.context,
        )
        try:
            ...
            self.assertEqual(structure.groups[0].channels[0].data_type, ods.DataTypeEnum.DT_LONGLONG)
            ...
            self.assertEqual(values.channels[0].values.data_type, ods.DataTypeEnum.DT_LONGLONG)
            self.assertSequenceEqual(values.channels[0].values.longlong_array.values, [1, 2, 3])
            self.assertEqual(values.channels[1].values.data_type, ods.DataTypeEnum.DT_DOUBLE)
            self.assertSequenceEqual(values.channels[1].values.double_array.values, [2.1, 2.2, 2.3])
    ...
    def test_comma_parameters(self):
        service = ExternalDataReader()
        handle = service.Open(
            exd_api.Identifier(url=self._get_example_file_path("example.csv"), parameters='{"sep":","}'), self.context
        )
        try:
            structure = service.GetStructure(exd_api.StructureRequest(handle=handle), self.context)
    ...
    def test_default(self):
        service = ExternalDataReader()
        handle = service.Open(
            exd_api.Identifier(url=self._get_example_file_path("example.csv"), parameters=None), self.context
        )
        try:
            structure = service.GetStructure(exd_api.StructureRequest(handle=handle), self.context)
            self.assertEqual(structure.name, "example.csv")
    ...
    def test_no_header(self):
        service = ExternalDataReader()
        handle = service.Open(
            exd_api.Identifier(url=self._get_example_file_path("example_no_header.csv"), parameters='{"header":null}'),
            self.context,
        )
        try:
            structure = service.GetStructure(exd_api.StructureRequest(handle=handle), self.context)
    ...
    def test_no_header2(self):
        ...
        handle = service.Open(
            exd_api.Identifier(
                ...
                parameters='{"header":null,"names":["d", "e", "f"]}',
            ),
            self.context,
        )
        try:
    ...
    def test_independent(self):
        ...
        try:
            ...
            self.assertIsNone(structure.groups[0].channels[1].attributes.variables.get("independent"))
    ...
    def test_not_my_file_2(self):
        ...
        handle = service.Open(
            exd_api.Identifier(
                url=self._get_example_file_path("example_semicolon.csv"), parameters=self._b64_param('{"sep": ";"}')
            ),
            context,
        )
        try:
    ...
    def test_not_my_file1(self):
        ...
        service = ExternalDataReader()
        handle = service.Open(
            exd_api.Identifier(url=self._get_example_file_path("not_my_file1.csv"), parameters='{"sep":","}'), context
        )
        try:
            with self.assertRaises(grpc.RpcError) as _:
                service.GetStructure(exd_api.StructureRequest(handle=handle), context)
    ...
    def test_not_my_file2(self):
        ...
        service = ExternalDataReader()
        handle = service.Open(
            exd_api.Identifier(url=self._get_example_file_path("not_my_file2.csv"), parameters='{"sep":","}'), context
        )
        try:
            with self.assertRaises(grpc.RpcError) as _:
                service.GetStructure(exd_api.StructureRequest(handle=handle), context)
